...
sdr_module          = SDRModule(sample_rate = 2.4e6, center_freq = 433e6, gain = 3, default_sample_number = 256)
...
forward_pid         = PIDController(Kp = 1.2, Ki = 0.15, Kd = 0, target = 0)
    ...
    while True:
        if system_enable:
            if first_run_flag:
                ...
                turn_angle = (60 - (((best_angle % 360) + 360) % 360) // 60 * 60) % 180
                vehicle.turn(turn_angle)

                dir = not (best_angle in [0.0, -60.0, -120.0])
                vehicle.move(2, dir)
                ...
                best_angle, best_signal_power = half_scan(30, move_time)
                gyro.calibrate(250)
...
def half_scan(scan_angle, move_time):
    ...
    bluetooth_output_prio = "H" + str(scan_angle)
    bluetooth_send_flag = True
